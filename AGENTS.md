...
- `run.py` is the ASE/metatomic molecular-dynamics entry point.
- `utils/` contains workflow configuration and I/O helpers, PET model export,
  methane insertion, harmonic heat-capacity analysis, the legacy i-PI/LAMMPS
  runner, and the visualization notebook.
...
- Preserve the directory layout above. When moving a file, update Python
  imports, TOML paths, shell commands, Markdown links, and this guide together.
- Keep generated weights, converted models, trajectories, logs, Slurm output,
  notebooks' generated state, and caches out of source control.
- Use `python -m compileall -q run.py utils` after Python changes and
...
- A lightweight dependency-independent smoke check is
  `python utils/insert_methane.py --dry-run`. Full MD requires the configured
