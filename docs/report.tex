...
MD loads the metatomic export
\texttt{pet-mad-1.5-s\_40nn\_nostress.pt} through
\texttt{metatomic-ase}. Harmonic analysis converts the matching checkpoint to
a PET-JAX representation. Using the same learned parameters in both paths is
essential: a trajectory from one potential must not be differentiated with a
different potential. The checkpoint is marked \texttt{nostress}; the current
workflow therefore keeps the cell fixed and does not assume reliable
pressure or cell derivatives. PET-MAD is a universal model rather than a
MOF-5-specific fit, so energy, force, and especially curvature accuracy for
the relevant structures must be validated \cite{petmad}.
...
\begin{itemize}
  ...
  \item The \texttt{nostress} checkpoint and fixed cell preclude the paper's
        constant-pressure cell fluctuations and thermal expansion.
