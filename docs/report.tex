...
\begin{equation}
  ...
  \left(\bm F^{\mathrm T}\bm F+\lambda\bm I\right)^{-1}
  \bm f(\bm x).
...
\begin{equation}
  ...
  \bm w_0,
  \alpha^2\left(\bm F^{\mathrm T}\bm F+\lambda\bm I\right)^{-1}
  \right],
  \qquad V^{(i)}(\bm x)=\bm w_i^{\mathrm T}\bm f(\bm x),
...
\begin{equation}
  ...
  =\frac{\sum_t\exp[-\beta\Delta V_t^{(i)}]H_t^{(i)}}
  {\sum_t\exp[-\beta\Delta V_t^{(i)}]},
...
\begin{equation}
  C_{P,\mathrm{cl}}^{(i)}(T)
  =\frac{\mathrm d\langle H^{(i)}\rangle_i^{\mathrm{CEA}}}{\mathrm dT},
  \qquad
  \sigma_{\mathrm{LLPR},C_P}(T)
  =\sqrt{\frac{1}{M-1}\sum_{i=1}^{M}
  \left[C_{P,\mathrm{cl}}^{(i)}(T)-\overline C_{P,\mathrm{cl}}(T)\right]^2}.
...
\begin{equation}
  \sigma_{\mathrm{hyb}}(T)=\sqrt{
    \sigma_{\mathrm{MD}}^2(T)
    +\sigma_{\mathrm{har}}^2(T)
    +\sigma_{\mathrm{LLPR},C_P}^2(T)},
...
The uncertainty calculations and their stored locations are as follows:
...
The LLPR/CEA contribution currently applies only to the classical NPT term;
member-resolved geometry optimization and PET-JAX Hessians are not implemented.
