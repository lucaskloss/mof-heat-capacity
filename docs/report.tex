...
therefore provides a defensible harmonic route for pristine MOF-5 and a
diagnostic local-curvature analysis for MOF-5 containing 100 methane
molecules; it does not reproduce the published path-integral \cpheat. Five
short, \SI{2}{ps} NVT validation trajectories at 100, 200, 300, 400, and
\SI{500}{K} have been completed. They demonstrate execution of the 924-atom
loaded system, but are too short to establish equilibration or statistical
convergence. This report sets out the theory, implementation, interpretation,
and convergence tests required to turn the current validation workflow into a
...
\subsection{Classical NVT molecular dynamics}

The statistical and dynamical foundations of classical MD are summarized in
Section~\ref{sec:classical-md-theory}. In the present workflow, initial
velocities are sampled from a Maxwell--Boltzmann distribution at the target
temperature and the center-of-mass momentum is removed. The current code does
not expose or record a velocity seed, so exact dynamical replicas are not yet
reproducible.

Temperature is controlled with ASE's Langevin integrator \cite{ase}, using
$\gamma=\SI{0.01}{fs^{-1}}$ in Eq.~\eqref{eq:langevin}. The friction time
$\gamma^{-1}=\SI{100}{fs}$ happens to equal the thermostat relaxation time
used in the reference study, although Langevin and Nos\'e--Hoover-chain
dynamics are not equivalent.

The completed loaded validation configurations use 4,000 steps with a
\SI{0.5}{fs} timestep, for \SI{2}{ps} at each of 100, 200, 300, 400, and
\SI{500}{K}. Positions, cell, energies, and momenta are written every step,
giving 4,001 trajectory frames including the initial configuration. The fixed
cell and NVT thermostat mean that pressure, thermal expansion, and \cpheat{}
are outside the present MD model.
...
\section{Completed validation campaign}

Five independent Slurm jobs were run for the same 100-methane starting
structure. The log files show that all jobs reached their requested 4,000 MD
steps and produced 4,001 frames. Table~\ref{tab:completed-md} reports only
directly observed endpoint information; it is not an equilibration analysis.

\begin{table}[htbp]
\centering
\begin{tabular}{@{}rrrrr@{}}
\toprule
Target $T$ (K) & Initial $T$ (K) & Final $T$ (K) & Duration (ps) & Frames \\
\midrule
100 & 95.4  & 99.2  & 2.0 & 4001 \\
200 & 215.7 & 207.0 & 2.0 & 4001 \\
300 & 296.3 & 300.7 & 2.0 & 4001 \\
400 & 400.3 & 399.4 & 2.0 & 4001 \\
500 & 484.2 & 495.5 & 2.0 & 4001 \\
\bottomrule
\end{tabular}
\caption{Completed short NVT validation trajectories for MOF-5 plus 100 methane molecules. Temperatures are instantaneous values from the first and final log records.}
\label{tab:completed-md}
\end{table}

The final temperatures are close to their targets, and successful completion
shows that the structure, model export, CUDA calculator, neighbor-list backend,
thermostat, and trajectory output operate together for 924 atoms. However,
\SI{2}{ps} is only 20 thermostat relaxation times and is far shorter than the
25--\SI{100}{ps} discarded equilibration intervals in the reference work.
Endpoint agreement with the thermostat does not demonstrate stationary
potential energy, equilibrated methane positions, structural stability, or
decorrelated sampling.

The heat-capacity submission script currently selects final frame 4000 and
writes a unique NPZ result for each MD temperature. This is an appropriate
resource and integration diagnostic. At the time of writing, no
\texttt{heat-capacity*.npz} files are present in the workspace, so no numerical
PET-MAD \cv{} values are reported here.
...
\begin{enumerate}
  \item Use the completed final-frame Hessian only to benchmark runtime,
        memory, and numerical stability.
  \item Extend MD until temperature, potential energy, framework descriptors,
        and methane distributions are stationary. Discard equilibration before
        selecting frames.
...
The active MD path is \texttt{run.py} with TOML configuration files. Methane
insertion is handled by
...
\end{verbatim}
Wall time and memory requests should be based on representative jobs rather
than extrapolated from the short MD runtime. Separate output directories and
...
\begin{itemize}
  ...
  \item The original fixed-cell trajectories preclude the paper's
        constant-pressure cell fluctuations and thermal expansion; the new NPT
        campaign must be converged and analyzed separately.
  \item Classical nuclei are used in MD; nuclear quantum effects enter only
        through the harmonic oscillator formula.
  \item The 2-ps loaded trajectories are validation runs, not equilibrated
        production samples.
  \item The random insertion is not an adsorption or Monte Carlo equilibrium
        distribution, and MD velocity seeds are not controlled.
  ...
  \item Averaging local harmonic frame curves is not an exact anharmonic \cv{}
        ...
        construction.
  \item Finite-size, sparse-Hessian, precision, model-validity, and statistical
        errors have not yet been quantified.
\end{itemize}
...
The repository now implements a complete technical path from periodic MOF-5
structures and methane insertion to GPU NVT trajectories and sparse-AD
harmonic \cv{} curves. The completed 100--\SI{500}{K} loaded trajectories
confirm that the 924-atom workflow executes successfully, but their
\SI{2}{ps} duration supports validation only. The most defensible first
quantitative target is a relaxed and converged harmonic \cv{} for pristine
MOF-5. Multiple optimized configurations or equilibrated trajectory frames can
then be used to assess variability in loaded MOF-5, with an explicit
local-harmonic label and statistical uncertainty. Quantitative reproduction of
Kapil \emph{et al.}'s loaded \cpheat{} requires a separate anharmonic,
constant-pressure, nuclear-quantum implementation.
