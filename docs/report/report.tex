...
ASDEX colors the columns of $\bm S$ so that two columns assigned the same
color do not have possible nonzero entries in the same row. If
$\mathcal C_a$ is one color class, SADMOF uses the compressed seed
\begin{equation}
...
\end{equation}
The known sparsity pattern makes the contributing column unambiguous in every
row, so the individual columns can be decoded from the single colored HVP.
The number of differentiated directions is therefore the number of colors,
rather than $3N$, and several colors may additionally be evaluated together
as one GPU batch.
...
construction \cite{sadMof}. The project's current value
\texttt{hops=3} is therefore a truncation and must be increased until the
frequencies and heat-capacity correction converge.
