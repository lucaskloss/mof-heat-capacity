"""Launch an i-PI server and its LAMMPS socket force clients."""
...
def _wait_for_socket(socket_path: Path, server: subprocess.Popen[object]) -> None:
    """Wait for i-PI to create its Unix-domain socket."""
...
def _stop_processes(processes: list[subprocess.Popen[object]]) -> None:
    """Terminate children that remain after a failed launch."""
...
def run_lammps_socket(xml_file: Path, lammps_input: Path, *, socket_name: str, clients: int,
    ...
    try:
        ...
        client_returncodes = [process.wait() for process in client_processes]
        if server_returncode != 0 or any(code != 0 for code in client_returncodes):
            raise RuntimeError(
                f"i-PI/LAMMPS failed: i-PI={server_returncode}, LAMMPS={client_returncodes}"
            )
