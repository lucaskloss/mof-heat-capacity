...
Options:
  ...
  --loading N          Methane loading used in generated names (default: 100).
...
if [[ ! "${LOADING}" =~ ^[1-9][0-9]*$ || ! "${REPLICAS}" =~ ^[1-9][0-9]*$ || ! "${CPUS_PER_TASK}" =~ ^[1-9][0-9]*$ ]]; then
...
cd "${PROJECT_DIR}"
echo "Protocol: MOF-5 + ${LOADING} CH4, ${MODEL}/${METHOD}; replicas=${REPLICAS}; temperatures=${TEMPERATURES[*]} K"
