...
# Submit the five short MOF-5 + 100 CH4 NVT validation jobs to Izar.
...
usage() {
    cat <<'EOF'
Usage: scripts/submit_mof5_100ch4_tests.sh [options]

Submit one 2 ps NVT job at each of 100, 200, 300, 400, and 500 K.
Run scripts/prepare_mof5_100ch4_tests.sh first.
...
echo "Submitting MOF-5 + 100 CH4 NVT tests"
...
for temperature in "${TEMPERATURES[@]}"; do
    ...
    if [[ ! -f "${config}" ]]; then
        if ((DRY_RUN)); then
            echo "warning: configuration is not prepared yet: ${config}" >&2
        else
            echo "error: configuration not found: ${config}" >&2
            echo "run scripts/prepare_mof5_100ch4_tests.sh first" >&2
            exit 2
        fi
    fi
    ...
    command=(
        sbatch
        --parsable
        --job-name="mof5-ch4-${temperature}K-test"
        --partition="${PARTITION}"
        --qos="${QOS}"
        --nodes=1
        --ntasks=1
